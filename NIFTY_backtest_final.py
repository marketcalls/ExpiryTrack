...
def build_dashboard(tdf: pd.DataFrame, total_pnl: float,
                    ...
                    peak_viol_dates: list, out_dir: Path) -> Path:
    """
    ...
    data_js = f"""
const CONFIG={{st_period:{ST_PERIOD},st_mult:{ST_MULTIPLIER},spread:{SPREAD_WIDTH},
lots:{NUM_LOTS},qty:{QTY},capital:{CAPITAL},target_pct:{TARGET_PCT},
max_loss_pct:{MAX_LOSS_PCT},config_str:{json.dumps(config_str)},
date_range:{json.dumps(date_range)}}};
const SUMMARY={{total_pnl:{round(total_pnl,0)},roi:{round(roi,1)},
wr:{round(wr,1)},pf:{round(pf,2)},mdd:{round(mdd,0)},
avg_win:{round(avg_win,0)},avg_loss:{round(avg_loss,0)},
rr_ratio:{round(rr_ratio,2)},n_trades:{n_tr},
n_wins:{len(wins)},n_losses:{len(losses)},peak_violations:{peak_viol}}};
...
function ft(filter,btn){
  ...
  document.getElementById('tlTb').innerHTML=src.map((t,i)=>`<tr>
    ...
    <td style="color:${t.re==='Target'?'var(--green)':t.re==='MaxLoss'?'var(--red)':'var(--text2)'}">${t.re}</td>
