"""
William Fractals Credit Spread Backtest — Full Data Validation + Rich Dashboard
================================================================================
SIGNAL ENGINE : William Fractals (period configurable, default 21 candles = n=10)
STRATEGY      : Same credit-spread entry/exit as SuperTrend version
  - Up Fractal   (bearish signal) → BEAR_CALL spread (sell ATM CE, buy OTM CE)
  - Down Fractal (bullish signal) → BULL_PUT spread  (sell ATM PE, buy OTM PE)
NO LOOK-AHEAD : Fractal confirmed only after n right-side candles close.
                Entry on the CLOSE of the confirmation candle (bar i).
FEATURES:
  [1] Dynamic output folder: backtest_WF{P}_{W}pt_{L}lots_{start}_{end}/
  ...
  [5] Signal audit log — every fractal recorded with outcome
...
Usage:
    uv run python NIFTY_backtest_fractals.py
"""
...
# ── STRATEGY CONFIG ────────────────────────────────────────────────────────────
# William Fractals config
# FRACTAL_N = 10  →  21-candle fractal (n left + 1 centre + n right)
# TradingView "Fractals (21)" = n=10
FRACTAL_N      = 10        # candles on each side of the fractal centre
TIMEFRAME      = "5min"
...
WARMUP_BARS_REQUIRED = FRACTAL_N * 10   # generous warmup: 10 full fractal windows
...
def make_output_dir(trade_start: str, trade_end: str) -> Path:
    ...
    Creates and returns a uniquely-named output directory:
      backtest_WF{N}_{W}pt_{L}lots_{start}_{end}/
    e.g.  backtest_WF10_500pt_3lots_gap200pts_wait10min_2024-10-01_2026-03-17/
    """
    name = (
        f"backtest_WF{FRACTAL_N}_"
...
# ══════════════════════════════════════════════════════════════════════════════
# WILLIAM FRACTALS (no look-ahead)
# ══════════════════════════════════════════════════════════════════════════════

def compute_fractals(highs: np.ndarray, lows: np.ndarray, n: int):
    """
    Compute William Fractals on a pre-built OHLC array — with deduplication.

    DEDUPLICATION FIX
    -----------------
    Multiple adjacent bars can each qualify as fractal centres (near-plateau),
    producing multiple confirm_bar signals from what TradingView shows as ONE
    triangle.  This caused 4-7 trades/day instead of 1-2.

    Fix: also return the centre bar index per confirm bar.  The simulation loop
    skips any signal whose centre index was already acted on — matching
    TradingView's one-marker-per-unique-extreme behaviour exactly.

    Parameters
    ----------
    highs, lows : np.ndarray  (full dataset length)
    n           : int         candles on each side (n=10 -> 21-candle fractal)

    Returns
    -------
    up_fractal        : np.ndarray[bool]  — True at confirmation bar
    down_fractal      : np.ndarray[bool]  — True at confirmation bar
    up_fractal_centre : np.ndarray[int]   — centre bar index (-1 = no signal)
    dn_fractal_centre : np.ndarray[int]   — centre bar index (-1 = no signal)
    """
    size              = len(highs)
    up_fractal        = np.zeros(size, dtype=bool)
    down_fractal      = np.zeros(size, dtype=bool)
    up_fractal_centre = np.full(size, -1, dtype=int)
    dn_fractal_centre = np.full(size, -1, dtype=int)

    for centre in range(n, size - n):
        confirm_bar = centre + n

        # Up Fractal: centre high strictly highest in 2n+1 window
        if highs[centre] > np.max(highs[centre - n: centre]) and \
           highs[centre] > np.max(highs[centre + 1: centre + n + 1]):
            up_fractal[confirm_bar]        = True
            up_fractal_centre[confirm_bar] = centre

        # Down Fractal: centre low strictly lowest in 2n+1 window
        if lows[centre] < np.min(lows[centre - n: centre]) and \
           lows[centre] < np.min(lows[centre + 1: centre + n + 1]):
            down_fractal[confirm_bar]      = True
            dn_fractal_centre[confirm_bar] = centre

    return up_fractal, down_fractal, up_fractal_centre, dn_fractal_centre
...
def run_simulation(df, provider, trade_start_date, trade_end_date):
    ...
    # ── William Fractals (NO look-ahead) ──────────────────────────────────────
    # up_fractal[i]   = True → bearish → BEAR_CALL  (up fractal = local high)
    # down_fractal[i] = True → bullish → BULL_PUT   (down fractal = local low)
    up_fractal, down_fractal, up_frac_ctr, dn_frac_ctr = compute_fractals(high, low, FRACTAL_N)
    ...
    last_fractal_dir = None   # tracks last confirmed fractal direction for DailyReEntry
    last_up_centre   = -1     # dedup: centre bar index of last acted-on up fractal
    last_dn_centre   = -1     # dedup: centre bar index of last acted-on down fractal
    signal_log = []
    ...
    for i in range(start_idx, end_idx):
        ...
        #     (same as live: fractal direction at 09:15 is stale; gap bar must close first)
        if (in_trade
                and t == TRADE_START
        ...
        # A signal fires only if its centre bar is NEW (not the same physical
        # extreme as the last signal we acted on).  This matches TradingView's
        # one-triangle-per-unique-extreme behaviour and prevents the 4-7 trades/day
        # overtrading caused by adjacent bars sharing the same fractal extreme.
        any_flip = None
        if up_fractal[i]:
            centre = up_frac_ctr[i]
            if centre != last_up_centre:   # new unique up-fractal extreme
                any_flip = "BEARISH"
                total_signals  += 1
                last_up_centre  = centre   # mark as seen
        elif down_fractal[i]:
            centre = dn_frac_ctr[i]
            if centre != last_dn_centre:   # new unique down-fractal extreme
                any_flip = "BULLISH"
                total_signals  += 1
                last_dn_centre  = centre   # mark as seen
            ...
            if sp is not None and bp is not None:
                ...
                if ec > 0 and (ec - curr_spread) >= ec * TARGET_PCT:
                    ...
                    if exit_pnl:
                        ...
                        running_pnl += exit_pnl["total_pnl"]
                        in_trade = False
                        # Re-enter immediately on same candle in last fractal direction
                        reentry_dir = last_fractal_dir if last_fractal_dir else any_flip
                        new_td = _try_enter(bar_time, bar_date, spot,
                                            reentry_dir, "Target-ReEntry")
                        if new_td:
                            trade_data = new_td
                            in_trade   = True
                            target_reentries += 1
        ...
        if any_flip and not in_trade:
            last_fractal_dir = any_flip   # remember for DailyReEntry
        ...
        # For fractals: re-enter in direction of the last confirmed fractal signal.
        # If no fractal has fired yet today, skip (unlike ST which always has a direction).
        if not in_trade and t == TRADE_START:
            ...
            elif last_fractal_dir is not None:
                gap_reentry_wait_until = None
                pending_flip = None
                new_td = _try_enter(bar_time, bar_date, spot,
                                    last_fractal_dir, "DailyReEntry")
                if new_td:
                    trade_data        = new_td
                    in_trade          = True
                    reentry_when_flat += 1
...
def build_dashboard(tdf: pd.DataFrame, total_pnl: float,
    ...
    config_str = (f"WF(n={FRACTAL_N}, {2*FRACTAL_N+1}-candle) · {TIMEFRAME} · "
...
<div class="header">
  <h1>William Fractals Credit Spread <span>Backtest</span></h1>
  <p id="cfgLine"></p>
...
document.getElementById('pageTitle').textContent=
  `Backtest WF(n=${CONFIG.fractal_n}, ${CONFIG.fractal_candles}-candle) ${CONFIG.spread}pt`;
document.getElementById('cfgLine').textContent=CONFIG.config_str;
document.getElementById('ftLeft').textContent=
  `WF(n=${CONFIG.fractal_n}) | NIFTY Weekly | ${CONFIG.spread}pt | ${CONFIG.lots}lots`;
...
${[
  ['Fractal N (each side)',  CONFIG.fractal_n],
  ['Fractal Window',         CONFIG.fractal_candles+' candles'],
  ...
  ['Warmup Bars',          CONFIG.warmup_bars+' (fractal_n x 10)'],
...
if __name__ == "__main__":
    print("=" * 90)
    print("WILLIAM FRACTALS CREDIT SPREAD BACKTEST — VALIDATED + RICH DASHBOARD")
    print("=" * 90)
    print(f"  WF(n={FRACTAL_N}, {2*FRACTAL_N+1}-candle) | {TIMEFRAME} | {SPREAD_WIDTH}pt | "
