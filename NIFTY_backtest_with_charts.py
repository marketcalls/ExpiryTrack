...
# Chart generation (Phase 4) — optional; skipped gracefully if plotly not installed
try:
...
# Set True  → auto-generate per-day trade verification charts after backtest.
# Set False → skip chart phase entirely (faster, no plotly dependency needed).
GENERATE_CHARTS   = True
CHARTS_MAX_TRADES = 5   # max option-candle rows per day chart (keep ≤ 5)
...
def run_simulation(df, provider, trade_start_date, trade_end_date):
    ...
    for i in range(start_idx, end_idx):
            ...
            if any_flip:
                ...
                if should_exit:
                    ...
                    if exit_pnl:
                        ...
                        running_pnl += exit_pnl["total_pnl"]
                        in_trade = False
                        # Enter on NEXT candle — store direction in pending_flip
            ...
            if sp is not None and bp is not None:
                ...
                if ec > 0 and (ec - curr_spread) >= ec * TARGET_PCT:
                    ...
                    if exit_pnl:
                        ...
                        running_pnl += exit_pnl["total_pnl"]
                        in_trade = False
                        # Re-enter immediately on same candle in current ST direction
        ...
        # Flip when flat — defer entry to NEXT candle via pending_flip
        if any_flip and not in_trade:
            ...
            if gap_reentry_wait_until and bar_time < gap_reentry_wait_until:
                ...
                pending_flip = any_flip          # will be picked up on next bar
...
# ══════════════════════════════════════════════════════════════════════════════
# PHASE 4 — PER-DAY TRADE VERIFICATION CHARTS
# ══════════════════════════════════════════════════════════════════════════════

def _resample_5m_per_day(df_1m):
    """Resample 1-min to 5-min bars day-by-day (avoids cross-day bars)."""
    parts = []
    for _date, g in df_1m.groupby(df_1m.index.date):
...
def _load_option_candles_for_chart(contract_key, date, con):
    """Fetch option 1-min OHLC for a single day from expirytrack."""
    d0 = date.isoformat()
    d1 = (date + timedelta(days=1)).isoformat()
    df = con.execute(f"""
...
def _find_contract_key_for_chart(contracts_df, strike, opt_type, expiry):
    expiry = pd.Timestamp(expiry)
    m = contracts_df[
        (contracts_df["strike_price"].astype(float) == float(strike)) &
        (contracts_df["contract_type"] == opt_type) &
        (contracts_df["expiry_date"] == expiry)
    ]
    return None if m.empty else m.iloc[0]["expired_instrument_key"]


def _build_day_chart(date, day_trades, nifty_5m, st_dir_series, st_val_series,
                     contracts_df, opt_con):
    """Build a Plotly figure for one trading day."""
    day_5m     = nifty_5m[nifty_5m.index.date == date]
    day_st_dir = st_dir_series[st_dir_series.index.date == date]
    day_st_val = st_val_series[st_val_series.index.date == date]

    if day_5m.empty:
        return None

    n_trades    = len(day_trades)
    show_trades = min(n_trades, CHARTS_MAX_TRADES)
    n_rows      = 2 + show_trades
    row_heights = [0.40, 0.10] + ([0.50 / show_trades] * show_trades if show_trades else [])
    if show_trades == 0:
        row_heights = [0.70, 0.30]

    subtitles = [
        f"NIFTY 50 (5min) + SuperTrend({ST_PERIOD},{ST_MULTIPLIER})",
    ...
    for i, (_, t) in enumerate(day_trades.head(show_trades).iterrows()):
        ...
        subtitles.append(
            ...
            f"{t['exit_reason']} | ₹{t['total_pnl']:+,.0f}"
        )

    v_space = min(0.02, 0.9 / max(n_rows - 1, 1))
    fig = make_subplots(
        rows=n_rows, cols=1,
        shared_xaxes=True,
    ...
    # ── Row 1: NIFTY 5-min candles ──────────────────────────────────────────
    fig.add_trace(go.Candlestick(
        x=day_5m.index, open=day_5m["open"], high=day_5m["high"],
        low=day_5m["low"], close=day_5m["close"], name="NIFTY",
    ...
    if not day_st_val.empty:
        bull_m = day_st_dir == 1
        bear_m = day_st_dir == -1
        bv = day_st_val.copy(); bv[bear_m] = np.nan
        fig.add_trace(go.Scatter(x=bv.index, y=bv.values, mode="lines",
            line=dict(color="#26a69a", width=2), name="ST Bull"), row=1, col=1)
        rv = day_st_val.copy(); rv[bull_m] = np.nan
        fig.add_trace(go.Scatter(x=rv.index, y=rv.values, mode="lines",
            line=dict(color="#ef5350", width=2), name="ST Bear"), row=1, col=1)
    ...
    for i, (_, t) in enumerate(day_trades.iterrows()):
        ...
        col_e   = "#26a69a" if is_bull else "#ef5350"
        sym_e   = "triangle-up" if is_bull else "triangle-down"
        fig.add_trace(go.Scatter(
            x=[entry_t], y=[t["entry_spot"]],
            mode="markers+text",
            marker=dict(symbol=sym_e, size=16, color=col_e,
                        line=dict(width=1, color="white")),
            text=[f"T{i+1}"], textposition="top center",
            textfont=dict(size=10, color="white"),
            name=f"T{i+1} Entry", showlegend=False,
            hovertext=(f"{'Bull Put' if is_bull else 'Bear Call'}<br>"
                       f"Spot: {t['entry_spot']:.0f}<br>"
                       f"Sell ₹{t['sell_entry']:.2f} | Buy ₹{t['buy_entry']:.2f}<br>"
                       f"Credit: ₹{t['net_credit']:.2f} | Qty: {int(t['qty'])}"),
        ), row=1, col=1)
        if exit_t.date() == date:
            ec = "#26a69a" if t["total_pnl"] > 0 else "#ef5350"
            fig.add_trace(go.Scatter(
                ...
                hovertext=(f"Exit: {t['exit_reason']}<br>"
                           f"P&L: ₹{t['total_pnl']:+,.0f} | Days: {t['days_held']}"),
            ), row=1, col=1)

    # ── Row 2: ST direction bar ──────────────────────────────────────────────
    if not day_st_dir.empty:
        bar_colors = ["#26a69a" if d == 1 else "#ef5350" for d in day_st_dir.values]
        fig.add_trace(go.Bar(
            x=day_st_dir.index, y=day_st_dir.values,
            marker_color=bar_colors, name="ST Dir", showlegend=False,
        ), row=2, col=1)
    ...
    for i, (_, t) in enumerate(day_trades.head(show_trades).iterrows()):
        ...
        entry_t  = pd.Timestamp(t["entry_time"])
        exit_t   = pd.Timestamp(t["exit_time"])
        sell_key = _find_contract_key_for_chart(
            contracts_df, t["sell_strike"], ot, t["expiry"])
        if sell_key:
            sdf = _load_option_candles_for_chart(sell_key, date, opt_con)
            if sdf is not None and not sdf.empty:
                fig.add_trace(go.Candlestick(
                    x=sdf.index, open=sdf["open"], high=sdf["high"],
                    low=sdf["low"], close=sdf["close"],
                    name=f"SELL {ot} {int(t['sell_strike'])}", showlegend=False,
                    increasing_line_color="#26a69a", decreasing_line_color="#ef5350",
                ), row=opt_row, col=1)
                ...
                fig.add_hline(y=t["sell_exit"], line_dash="dot",
                              ...
                              row=opt_row, col=1)
                fig.add_trace(go.Scatter(
                    x=[entry_t], y=[t["sell_entry"]], mode="markers",
                    marker=dict(symbol="triangle-down", size=14, color="#ef5350",
                                line=dict(width=1, color="white")),
                    showlegend=False,
                    hovertext=(f"SELL {ot} {int(t['sell_strike'])} @ ₹{t['sell_entry']:.2f}<br>"
                               f"BUY {ot} {int(t['buy_strike'])} @ ₹{t['buy_entry']:.2f}<br>"
                               f"Net Credit: ₹{t['net_credit']:.2f}"),
                ), row=opt_row, col=1)
                if exit_t.date() == date:
                    ec = "#26a69a" if t["total_pnl"] > 0 else "#ef5350"
                    fig.add_trace(go.Scatter(
                        x=[exit_t], y=[t["sell_exit"]], mode="markers",
                        marker=dict(symbol="star", size=14, color=ec,
                                    line=dict(width=1, color="white")),
                        showlegend=False,
                        hovertext=(f"Buy-back @ ₹{t['sell_exit']:.2f}<br>"
                                   f"{t['exit_reason']} | P&L: ₹{t['total_pnl']:+,.0f}"),
                    ), row=opt_row, col=1)
    ...
    ov_cnt  = (day_trades["days_held"] > 0).sum()
    pc      = "green" if day_pnl > 0 else "red"
    ov_tag  = f" | {ov_cnt} overnight" if ov_cnt > 0 else ""

    fig.update_layout(
        title=dict(
            text=(f"<b>{date.strftime('%A %d %b %Y')}</b> | "
                  f"ST({ST_PERIOD},{ST_MULTIPLIER}) | "
                  f"{n_trades} trade{'s' if n_trades != 1 else ''}{ov_tag} | "
                  f"W:{wins} L:{losses} | "
                  f"<span style='color:{pc}'>Net P&L: ₹{day_pnl:,.0f}</span>"),
            font=dict(size=16),
        ),
    ...
    for ax_i in range(1, n_rows + 1):
        axname = f"xaxis{ax_i}" if ax_i > 1 else "xaxis"
        fig.update_layout(**{axname: dict(rangeslider=dict(visible=False))})

    return fig


def _build_trade_table_html(day_trades):
    rows = ""
    for _, t in day_trades.iterrows():
        cls  = "profit" if t["total_pnl"] > 0 else "loss"
        ot   = "PE" if t["type"] == "BULL_PUT" else "CE"
        hold = f"{t['days_held']}D" if t["days_held"] > 0 else "Intraday"
        en   = pd.Timestamp(t["entry_time"]).strftime("%Y-%m-%d %H:%M")
        ex   = pd.Timestamp(t["exit_time"]).strftime("%Y-%m-%d %H:%M")
        rows += (
            f"<tr>"
            f"<td>{t.get('trade_no','')}</td><td>{en}</td><td>{ex}</td>"
            f"<td>{t['type']}</td>"
            f"<td>{ot} {int(t['sell_strike'])}</td>"
            f"<td>{ot} {int(t['buy_strike'])}</td>"
            f"<td>{t['entry_spot']:.0f}</td>"
            f"<td>{t['sell_entry']:.2f}</td><td>{t['buy_entry']:.2f}</td>"
            f"<td>{t['net_credit']:.2f}</td>"
            f"<td>{t['sell_exit']:.2f}</td><td>{t['buy_exit']:.2f}</td>"
            f"<td>{t['exit_spread']:.2f}</td>"
            f"<td>{hold}</td>"
            f"<td>₹{t['gross_pnl']:,.0f}</td>"
            f"<td>₹{t['charges']:,.0f}</td>"
            f"<td class='{cls}'>₹{t['total_pnl']:,.0f}</td>"
            f"<td>{t['exit_reason']}</td>"
            f"<td>{int(t['qty'])}</td>"
            f"<td>₹{int(t['margin_req']):,}</td>"
            f"</tr>"
        )
    return rows


def _generate_index_html(all_days, charts_dir, total_journal_trades):
    rows = cum = ""
    cum_pnl = 0
    for date, info in sorted(all_days.items()):
        ...
        ov  = f" ({info['overnight']}ON)" if info["overnight"] > 0 else ""
        fname_val = info["filename"]
        rows += (
            f"<tr>"
            f"<td><a href='{fname_val}'>{date.strftime('%Y-%m-%d')}</a></td>"
            f"<td>{date.strftime('%A')}</td>"
            f"<td>{info['trades']}{ov}</td>"
            f"<td>{info['wins']}</td><td>{info['losses']}</td>"
            f"<td class='{pc}'>₹{info['pnl']:,.0f}</td>"
            f"<td class='{cc}'>₹{cum_pnl:,.0f}</td>"
            f"</tr>"
        )

    total_t   = sum(d["trades"] for d in all_days.values())
    total_pnl = sum(d["pnl"]    for d in all_days.values())
    total_w   = sum(d["wins"]   for d in all_days.values())

    html = f"""<!DOCTYPE html>
<html><head>
<title>ST({ST_PERIOD},{ST_MULTIPLIER}) Trade Charts</title>
<style>
...
<h1>SuperTrend({ST_PERIOD},{ST_MULTIPLIER}) — Trade Verification Charts</h1>
<h2>NIFTY {SPREAD_WIDTH}pt Spread | {NUM_LOTS} lots × {LOT_SIZE} = {QTY} qty</h2>
<div class="summary">
  <div class="stat"><div class="stat-value">{len(all_days)}</div>
    <div class="stat-label">Trading Days</div></div>
  <div class="stat"><div class="stat-value">{total_t}</div>
    <div class="stat-label">Trades (charts)</div></div>
  <div class="stat"><div class="stat-value">{total_journal_trades}</div>
    <div class="stat-label">Total in Journal</div></div>
  <div class="stat"><div class="stat-value">{total_w}</div>
    <div class="stat-label">Winners</div></div>
  <div class="stat">
    <div class="stat-value {'profit' if total_pnl>0 else 'loss'}">₹{total_pnl:,.0f}</div>
    <div class="stat-label">Net P&L</div></div>
</div>
<p>Click any date to view NIFTY 5min + SuperTrend + sell-leg option candles with entry/exit markers.</p>
<table>
<tr><th>Date</th><th>Day</th><th>Trades</th>
    <th>Wins</th><th>Losses</th><th>Day P&L</th><th>Cum P&L</th></tr>
{rows}
</table>
</body></html>"""

    idx_path = charts_dir / "index.html"
    idx_path.write_text(html, encoding="utf-8")
    return idx_path


def generate_trade_charts(tdf: pd.DataFrame, out_dir: Path) -> Path:
    """
    Phase 4 — generate one HTML chart per entry-day + an index.html.
    Returns path to index.html inside out_dir/charts/.
    """
    ...
    journal["exit_time"]  = pd.to_datetime(journal["exit_time"])
    ...
    print(f"  {total_journal_trades} trades | {len(trade_dates)} entry days", flush=True)

    # Load full NIFTY 1-min for ST continuity (from warmup start)
    print("  Loading full NIFTY 1-min data for SuperTrend...", flush=True)
    opt_con  = duckdb.connect(DB_PATH, read_only=True)
    first_dt = journal["entry_time"].min() - pd.DateOffset(months=4)
    df_1m = opt_con.execute(f"""
        SELECT timestamp, open, high, low, close
        FROM candle_data
        WHERE instrument_key = 'NSE_INDEX|Nifty 50'
          AND interval = '1minute'
          AND timestamp >= '{first_dt.date()}'
        ORDER BY timestamp
    """).fetchdf()
    ...
    print(f"    {len(df_1m):,} 1-min bars loaded", flush=True)

    nifty_5m = _resample_5m_per_day(df_1m)
    print(f"    {len(nifty_5m):,} 5-min bars", flush=True)

    # Compute SuperTrend on full series (continuity)
    st_dir_arr, st_val_arr = compute_supertrend(
        nifty_5m["high"].values.astype(float),
        nifty_5m["low"].values.astype(float),
        nifty_5m["close"].values.astype(float),
        ST_PERIOD, ST_MULTIPLIER,
    )
    st_dir_series = pd.Series(st_dir_arr, index=nifty_5m.index)
    st_val_series = pd.Series(st_val_arr,  index=nifty_5m.index)

    # Load option contracts for chart key lookup
    contracts_df = opt_con.execute("""
        SELECT expired_instrument_key, expiry_date, contract_type, strike_price
        FROM contracts
        WHERE instrument_key = 'NSE_INDEX|Nifty 50'
          AND data_fetched = true AND no_data = false
    """).fetchdf()
    contracts_df["expiry_date"] = pd.to_datetime(contracts_df["expiry_date"])
    print(f"    {len(contracts_df):,} contracts loaded", flush=True)

    # Generate one HTML per entry day
    all_days = {}
    print(f"  Generating {len(trade_dates)} day charts...", flush=True)
    ...
    for idx, date in enumerate(trade_dates):
        ...
        overnight= int((day_trades["days_held"] > 0).sum())
        fname    = f"{date.isoformat()}.html"

        fig = _build_day_chart(
            date, day_trades, nifty_5m,
            st_dir_series, st_val_series,
            contracts_df, opt_con,
        )
        if fig is None:
            print(f"    ⚠️  {date} — no 5-min data, skipped")
            continue

        table_rows = _build_trade_table_html(day_trades)
        ...
        page = f"""<!DOCTYPE html>
<html><head>
<title>ST Chart — {date.isoformat()}</title>
<style>
  ...
  th{{background:#16213e;color:#00d2ff;padding:8px;text-align:left;
...
{chart_html}
<h3>Trade Details — ST({ST_PERIOD},{ST_MULTIPLIER}) Credit Spread</h3>
<table>
<tr>
  <th>#</th><th>Entry</th><th>Exit</th><th>Spread</th>
  <th>Sell</th><th>Buy</th><th>Spot</th>
  <th>Sell Prem</th><th>Buy Prem</th><th>Credit</th>
  <th>Sell Exit</th><th>Buy Exit</th><th>Spread Out</th>
...
{table_rows}
        ...
        (charts_dir / fname).write_text(page, encoding="utf-8")
        ...
        if (idx + 1) % 25 == 0 or (idx + 1) == len(trade_dates):
            print(f"    {idx+1}/{len(trade_dates)} done...", flush=True)

    opt_con.close()

    idx_path = _generate_index_html(all_days, charts_dir, total_journal_trades)
    print(f"  ✅  {len(all_days)} charts saved → {charts_dir}/", flush=True)
    return idx_path
...
if __name__ == "__main__":
    print("=" * 90)
    print("SUPERTREND CREDIT SPREAD BACKTEST — VALIDATED + RICH DASHBOARD + CHARTS")
    print("=" * 90)
    print(f"  ST({ST_PERIOD}, {ST_MULTIPLIER}) | {TIMEFRAME} | {SPREAD_WIDTH}pt | "
          f"{NUM_LOTS}x{LOT_SIZE}={QTY} qty | Capital Rs {CAPITAL:,}")
    print(f"  Chart generation: {'ON' if GENERATE_CHARTS else 'OFF'}")
    ...
    # Save CSVs
    csv_name = (f"trades_ST{ST_PERIOD}_{ST_MULTIPLIER}_"
    ...
    # Phase 4: per-day trade charts
    charts_idx = None
    if GENERATE_CHARTS:
        print(f"\n{'='*90}")
        print("PHASE 4 — GENERATING PER-DAY TRADE CHARTS")
        print("=" * 90)
        charts_idx = generate_trade_charts(
            tdf.drop(columns=["month"], errors="ignore").copy(),
            out_dir,
        )
    ...
    if charts_idx:
        print(f"  └── charts/")
        print(f"        ├── index.html      ← browse all days")
        print(f"        └── YYYY-MM-DD.html ← one per entry day")
    print("\nDone.")
