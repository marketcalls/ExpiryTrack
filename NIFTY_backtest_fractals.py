...
def compute_fractals(highs: np.ndarray, lows: np.ndarray, n: int):
    """
    Compute William Fractals on a pre-built OHLC array.

    A fractal at bar `i` is confirmed only once bar `i+n` has CLOSED —
    i.e., the signal is available from bar index `i+n` onwards.
    We return arrays aligned to the full bar count where:

      up_fractal[j]   = True  → bar j-n is an UP fractal   (bearish → sell CALL)
      down_fractal[j] = True  → bar j-n is a DOWN fractal  (bullish → sell PUT)

    This means: at bar j we know the fractal that formed n bars ago.
    Entry is taken on the CLOSE of bar j (the confirmation candle).
    ...
    Returns
    -------
    up_fractal   : np.ndarray[bool]  — signal available at confirmation bar
    down_fractal : np.ndarray[bool]
    """
    size         = len(highs)
    up_fractal   = np.zeros(size, dtype=bool)
    down_fractal = np.zeros(size, dtype=bool)

    # Centre candle range: [n, size-n)
    # Confirmation available at bar: centre + n
    for centre in range(n, size - n):
        confirm_bar = centre + n          # earliest bar we can act on this signal

        # Up Fractal: centre high is strictly the highest in 2n+1 window
        if highs[centre] > np.max(highs[centre - n: centre]) and \
           highs[centre] > np.max(highs[centre + 1: centre + n + 1]):
            up_fractal[confirm_bar] = True

        # Down Fractal: centre low is strictly the lowest in 2n+1 window
...
def run_simulation(df, provider, trade_start_date, trade_end_date):
    ...
    up_fractal, down_fractal = compute_fractals(high, low, FRACTAL_N)
    ...
    for i in range(start_idx, end_idx):
        ...
        # ── FRACTAL signal detection (no look-ahead) ─────────────────────────
        # Signal is confirmed at bar i (after n right-side bars have closed).
        # Entry on the CLOSE of this bar — exactly like ST flip logic.
        any_flip = None
        if up_fractal[i]:
            any_flip = "BEARISH"; total_signals += 1   # up fractal → sell CE
        elif down_fractal[i]:
            any_flip = "BULLISH"; total_signals += 1   # down fractal → sell PE
