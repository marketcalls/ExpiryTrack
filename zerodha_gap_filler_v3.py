...
def diagnose_deep():
    ...
    if expiry_rows:
        ...
        # Check if ATM PE contract has data — use aggregate-safe query
        row = con.execute("""
        ...
        bars = row[0] if row else 0
        if bars > 0:
            print(f"    ATM PE {atm} expiry {expiry}: {bars} bars on 2025-02-20  OK - option data present")
        else:
            print(f"    ATM PE {atm} expiry {expiry}: 0 bars on 2025-02-20  <- OPTION DATA MISSING")

    print()
    print("  CONCLUSION:")
    print("    Spot data : 15 days missing for Feb-Mar 2025 gap  <- ROOT CAUSE")
    print("    Option data: fully present for both gaps")
    print("    Fix: python3 zerodha_gap_filler_v2.py --run-spot")
