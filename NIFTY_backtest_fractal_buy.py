...
FRACTAL_N       = 10        # n each side → 21-candle fractal (TV Fractals 21)
TIMEFRAME       = "5min"
STRIKE_INTERVAL = 50        # NIFTY strike spacing

# ── STRIKE SELECTION ─────────────────────────────────────────────────────────
# 1 = 1 strike ITM, 2 = 2 strikes ITM
# Down fractal → buy CE ITM:  strike = ATM - ITM_DEPTH * STRIKE_INTERVAL
# Up   fractal → buy PE ITM:  strike = ATM + ITM_DEPTH * STRIKE_INTERVAL
ITM_DEPTH = 1               # Try: 1 or 2

# ── POSITION SIZING ───────────────────────────────────────────────────────────
LOT_SIZE  = 65              # NIFTY lot size (check current — changed to 75 in 2024)
NUM_LOTS  = 10               # number of lots per trade
QTY       = LOT_SIZE * NUM_LOTS

# ── EXIT RULES ────────────────────────────────────────────────────────────────
TARGET_PCT   = 0.25         # exit when profit  >= 25% of entry premium  (e.g. 0.20, 0.25, 0.30)
MAX_LOSS_PCT = 0.50         # exit when loss    >= 25% of entry premium  (e.g. 0.20, 0.25, 0.30)

# Force intraday exit — set to None to allow overnight holds
# Set to dtime(15, 15) to always exit same day before expiry close
INTRADAY_EXIT  = dtime(15, 15)   # None = allow overnight, dtime(15,15) = intraday only

TRADE_START    = dtime(9, 15)
EXPIRY_EXIT    = dtime(15, 15)

# ── CAPITAL ───────────────────────────────────────────────────────────────────
CAPITAL = 400_000           # smaller capital needed for buying vs selling
...
WARMUP_BARS_REQUIRED = FRACTAL_N * 10
...
def make_output_dir(trade_start: str, trade_end: str) -> Path:
    name = (
        ...
        f"ITM{ITM_DEPTH}_"
        f"T{int(TARGET_PCT*100)}L{int(MAX_LOSS_PCT*100)}_"
        f"{NUM_LOTS}lots_"
    ...
    for i in range(start_idx, end_idx):
        ...
        if pending_flip and not in_trade and t >= TRADE_START:
            ...
            if new_td:
                ...
                pending_flip = None   # give up after 09:45
