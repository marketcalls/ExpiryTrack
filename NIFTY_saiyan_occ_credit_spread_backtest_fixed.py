...
print("\n" + "=" * 70)
print("  SAIYAN OCC CREDIT SPREAD RESULTS [BIAS-FIXED v2]")
...
print(f"  Fixes: Lookahead removed | Slippage={SLIPPAGE_PCT}% | Next-bar exec | Max-loss on gaps")
print("=" * 70)
...
print(f"""
  ...
  Still paper-trade for 2-4 weeks before going live to validate signal timing.
""")
...
fig.update_layout(
    title=f"Saiyan OCC Credit Spread [FIXED] | {SYMBOL} | {MA_TYPE}({MA_PERIOD}) x{ALT_TF_MULTIPLIER} | "
          f"Spread: {SPREAD_WIDTH}pt | {NUM_LOTS} lots | Slippage: {SLIPPAGE_PCT}%",
...
chart_path = os.path.join(os.path.dirname(__file__), "NIFTY_saiyan_occ_credit_spread_fixed_chart.html")
...
# EXPORT
# ============================================================================
trades_csv = script_dir / "NIFTY_saiyan_occ_credit_spread_fixed_trades.csv"
trades_df.to_csv(trades_csv, index=False)
print(f"Trades exported to: {trades_csv}")

equity_csv = script_dir / "NIFTY_saiyan_occ_credit_spread_fixed_equity.csv"
